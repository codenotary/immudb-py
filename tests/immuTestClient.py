...
class ImmuTestClient:
    ...
    def amIHigherOrEqualsToVersion(self, version: str):
    ...
    def createTestTable(self, *fields: List[str]):
        ...
        resp = self.client.sqlExec(
            "CREATE TABLE {table} ({fieldsJoined});".format(table = tabname, fieldsJoined = fieldsJoined)
        )
        assert((len(resp.txs) > 0 and not resp.ongoingTx and not resp.UnknownFields())
                   or len(resp.UnknownFields()) > 0)
    ...
    def prepareSelectQuery(self, fromWhat: str, whatToSelect: List[str], conditions: List[str]):
        ...
        constructed = "SELECT {fieldsJoined} FROM {table}".format(table=fromWhat, fieldsJoined = fieldsJoined)
        if(len(conditions) > 0):
    ...
    def simpleSelect(self, fromWhat: str, whatToSelect: List[str], params: dict, *conditions: List[str]):
        preparedQuery = self.prepareSelectQuery(fromWhat, whatToSelect, conditions)
